 ...
 */
async function providerWith(rpcFile: unknown, alchemyKey = ''): Promise<RpcProvider> {
  await writeFile(join(dir, 'rpc.json'), JSON.stringify(rpcFile), 'utf8')
  const p = new RpcProvider()
  await p.load(dir, alchemyKey)
  return p
}
...
describe('降级顺序', () => {
  ...
  it('Lark 没配就直接用 ChainList', async () => {
    const p = await providerWith({ syncedAt: '', lark: {}, chainlist: { morph: ['https://public'] } })
    expect(p.urlsFor(chain())).toEqual(['https://public'])
  })

  it('同一个 URL 在多个来源出现时只保留优先级最高的那次', async () => {
    const p = await providerWith({
  ...
  it('一个 RPC 都没有时抛错，而不是返回空数组让上层莫名其妙', async () => {
    const p = await providerWith({ syncedAt: '', lark: {}, chainlist: {} })
    expect(() => p.urlsFor(chain())).toThrow(/没有可用的 RPC/)
  })
})

describe('★ 私有 RPC 不下发前端', () => {
  it('带 query 参数的 Lark RPC 视为私有', async () => {
    const p = await providerWith({
      syncedAt: '',
  ...
  it('ChainList 的公开 RPC 全部可下发', async () => {
    const p = await providerWith({
  ...
  it('ChainList 里带 ${} 模板占位符的会被过滤（那不是能用的地址）', async () => {
    const p = await providerWith({
...
describe('诊断信息', () => {
  it('describe 报出每个来源各提供了几个', async () => {
    const p = await providerWith({
      syncedAt: '', lark: { morph: ['https://a'] }, chainlist: { morph: ['https://b', 'https://c'] },
    })
    const counts = p.describe(chain())
    expect(counts.lark).toBe(1)
    expect(counts.chainlist).toBe(2)
  })

  it('hasAny 不抛错，用于启动校验', async () => {
    const p = await providerWith({ syncedAt: '', lark: {}, chainlist: {} })
    expect(p.hasAny(chain())).toBe(false)
  })

  it('rpc.json 不存在时不崩，走空来源', async () => {
    const p = new RpcProvider()
    await p.load(dir, '') // 目录里没有 rpc.json
    expect(p.hasAny(chain())).toBe(false)
    expect(p.syncedAt).toBe('')
  })
})

describe('Alchemy 那一级', () => {
  it('配了 key 就排在 Lark 之后、ChainList 之前', async () => {
    const p = await providerWith(
  ...
  it('★ Alchemy 的 URL 含 key，永远不下发前端', async () => {
    const p = await providerWith({ syncedAt: '', lark: {}, chainlist: {} }, 'TESTKEY')
  ...
  it('Alchemy 不支持的链就不产出（不会拼一个必然 404 的地址）', async () => {
    ...
    const p = await providerWith({ syncedAt: '', lark: {}, chainlist: { weird: ['https://w'] } }, 'TESTKEY')
    expect(p.urlsFor(unknown)).toEqual(['https://w'])
  })

  it('没配 key 就整级跳过', async () => {
    const p = await providerWith({ syncedAt: '', lark: {}, chainlist: { morph: ['https://p'] } })
...
describe('★ Tron 不走 Alchemy', () => {
  it('Alchemy 有 tron 端点，但它讲 JSON-RPC 而 TronWeb 要 TronGrid REST，所以不列入候选', async () => {
    ...
    // 只有 TronGrid，没有 alchemy
    expect(p.urlsFor(tron)).toEqual(['https://api.trongrid.io'])
    expect(p.describe(tron).alchemy).toBe(0)
  })
})
